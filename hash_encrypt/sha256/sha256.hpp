...
//
		class Sha256 {
			...
			protected:
				...
				/** @brief:               SHA256算法对输入信息的预处理，包括“附加填
				                          充比特”和“附加长度值”

				附加填充比特:             在报文末尾进行填充，先补第一个比特为1，然
				                          后都补0，直到长度满足对512取模后余数是448。
										  需要注意的是，信息必须进行填充。

				附加长度值:               用一个64位的数据来表示原始消息（填充前的消
				                          息）的长度，并将其补到已经进行了填充操作的
										  消息后面。
				...
				*/
				...
				/** @brief: 由64Byte大小的数据块，构造出64个4Byte大小的字。
				            构造算法：前16个字直接由数据块分解得到，其余的字由如下迭
							代公式得到：W[t] = small_sigma1(W[t-2]) + W[t-7] + small
							_sigma0(W[t-15]) + W[t-16]
			...
			private:
				static std::vector<uint32_t> initial_message_digest_; // 在SHA256算
				// 法中的初始
				// 信息摘要，
				// 这些常量是
				// 对自然数中
				...
				static std::vector<uint32_t> add_constant_; // 在SHA256算法中，用到
				// 64个常量，这些常量是
				// 对自然数中前64个质数
				// 的立方根的小数部分取
				// 前32bit而来。
...
// 内联函数&模版函数的定义 /
	...
	class bad_encrypt: public runtime_error;
	}
